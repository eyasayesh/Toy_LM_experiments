...
class FreqWeightedTokenizer:
    ...
    def get_pair_frequencies(self, tokens_list: List[List[str]]) -> Counter:
        pairs = Counter()
    ...
    def train(self, text: str) -> None:
        ...
        for _ in range(num_merges):
            pair_freq = self.get_pair_frequencies(tokens_list)
            if not pair_freq:
                break
            pairs = list(pair_freq.keys())
            weights = list(pair_freq.values())
            pair = random.choices(pairs, weights=weights, k=1)[0]
