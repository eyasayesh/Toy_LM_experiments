...
class UniformPairTokenizer:
    """
    A variant of BPE tokenizer that samples uniformly over all pairs 
    rather than selecting the most frequent pairs.
    
    This tokenizer properly handles text as a single sequence and
    maintains consistent merge IDs in the tokens.
    """
    ...
    def get_pair_candidates(self, tokens: List[str]) -> Set[Tuple[str, str]]:
        """
        Get all unique adjacent token pairs from a list of tokens.
        
        Args:
            tokens: List of tokens
            
        Returns:
            Set of unique adjacent token pairs
        """
        if len(tokens) < 2:
            return set()
    ...
    def train(self, text: str) -> None:
        ...
        for i in range(num_merges):
            ...
            pairs = self.get_pair_candidates(tokens)
            if not pairs:
                break
                
            # Uniformly sample a pair to merge
            a, b = random.choice(list(pairs))
            
            # Create a unique merged token with an identifier to avoid ambiguity
            merged = f"{a}{b}@{i}"
